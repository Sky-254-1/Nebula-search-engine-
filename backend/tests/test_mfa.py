...
class TestMFAService:
    ...
    def test_generate_secret(self):
        ...
        assert len(secret) > 0
        # pyotp base32 secrets are uppercase letters and digits
    ...
    def test_get_totp_uri(self):
        ...
        uri = MFAService.get_totp_uri(
            "JBSWY3DPEHPK3PXP", "test@example.com", issuer="Nebula"
        )
        assert "test" in uri  # email is URL-encoded as test%40example.com
...
class TestVerifyMFAToken:
    ...
    def test_verify_mfa_token_backup_code_rejected(self):
        """Edge case: backup code token rejected (goes through endpoint)."""
        from app.services.mfa import verify_mfa_token
    ...
    def test_verify_mfa_token_removes_spaces(self):
        ...
        totp = pyotp.TOTP(secret)
        token = totp.now()
        # Space-separated version of the valid token
        spaced = token[:3] + " " + token[3:]
        is_valid, error = verify_mfa_token(secret, spaced)
        assert is_valid is True  # Spaces removed, valid 6-digit token
