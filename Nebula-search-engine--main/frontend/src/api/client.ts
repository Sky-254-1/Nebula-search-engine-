...
class APIClient {
  ...
  public async post<T>(url: string, data?: any, config?: { params?: Record<string, any> }): Promise<T> {
    const response = await this.client.post<T>(url, data, config ? { params: config.params } : undefined);
    return response.data;
  }
